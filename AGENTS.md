...
Project Armstrong (see the root `AGENTS.md` one level up) — nothing outside
...
The root `.Codex/skills/gsap-*` are the **official GreenSock skills**, MIT-licensed, vendored from
...
The root `.Codex/skills/lenis/` is **not vendored from an upstream skill** — darkroom.engineering
(Lenis's maintainer) publishes no `.Codex-plugin`, skill, or MCP. The only third-party
