...
// Section 1: Hero + Services Combo — Revamped Full-Screen HUD Layout with Center Focus & Edge Controls
function HeroAndServicesSection() {
  ...
  return (
    <Box
      ...
      sx={{
        ...
        alignItems: "center",
        justifyContent: "center",
        background: `radial-gradient(ellipse at center, #FFFFFF 25%, ${alpha(NOIR.panel, 0.8)} 65%, ${alpha(NOIR.navyField, 0.08)} 88%, ${alpha(NOIR.navyField, 0.16)} 100%)`,
        pt: { xs: 16, md: 12 },
        pb: { xs: 10, md: 6 },
        px: { xs: 4, md: 8 },
      }}
    >
      {/* 1. Full-Screen Interactive Signal Canvas Layer */}
      <Box aria-hidden sx={{ position: "absolute", inset: 0, zIndex: 0, opacity: { xs: 0.4, md: 0.85 } }}>
        <HeroSignalP />
      </Box>

      {/* 3. Subtle Corner Vignette (Noticeable only at extreme corners) */}
      <Box
        aria-hidden
        sx={{
          position: "absolute",
          ...
          zIndex: 1,
          pointerEvents: "none",
          background: `radial-gradient(ellipse at center, transparent 78%, ${alpha(NOIR.navyField, 0.08)} 92%, ${alpha(NOIR.navyField, 0.22)} 100%)`,
        }}
      />

      {/* 5. CENTERED HERO FOCAL POINT (Static Minimal Title & Tagline) */}
      <Container
        maxWidth={false}
        sx={{
          position: "relative",
        ...
        <Stack spacing={3} alignItems="center">
          ...
          >
            <Typography
              ...
              component="h1"
              sx={{
                fontSize: { xs: "3.6rem", sm: "5.4rem", md: "7.2rem" },
                fontWeight: 900,
                letterSpacing: "0.02em",
                lineHeight: 1,
                textTransform: "uppercase",
                userSelect: "none",
                textShadow: `0 0 35px ${alpha(NOIR.gold, 0.2)}`,
              }}
            >
              PH<Box component="span" sx={{ color: NOIR.gold, textShadow: `0 0 25px ${alpha(NOIR.gold, 0.6)}` }}>IT</Box>OPOLIS
            </Typography>
          </motion.div>

          {/* Tagline Pill Badge (Below PHITOPOLIS) */}
          <motion.div
          ...
          >
            <Box
              sx={{
                ...
                border: `1px solid ${alpha(NOIR.hairline, 0.8)}`,
                userSelect: "none",
                boxShadow: `0 4px 16px ${alpha(NOIR.navyField, 0.06)}`,
              }}
            >
              <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: NOIR.gold, boxShadow: `0 0 8px ${NOIR.gold}` }} />
              <Typography variant="overline" sx={{ fontFamily: MONO, letterSpacing: "0.18em", fontSize: "0.72rem", color: NOIR.ink, fontWeight: 600 }}>
      ...
      {/* 6. EDGE-ANCHORED HUD CORNER CONTROLS */}

      {/* Top-Left Corner HUD: Plain Text Description (Extended Horizontal Space) */}
      <Box
        sx={{
      ...
      >
        ...
        >
          {CONTENT.hero.description.split(/(R&D firm)/g).map((part, i) =>
            /^(R&D firm)$/.test(part) ? (
              <Box key={i} component="span" sx={{ color: NOIR.gold, fontWeight: 700 }}>
                {part}
              </Box>
            ) : (
              part
      ...
      {/* Top-Right Corner HUD: Contact Us Button (Less Round Border Radius) */}
      <Box
        sx={{
          position: { xs: "relative", md: "absolute" },
          top: { md: 80 },
      ...
      >
        <RouterButton
          ...
          sx={{
            ...
            fontFamily: MONO,
            letterSpacing: "0.08em",
            borderRadius: "4px", // Less round border radius
            boxShadow: `0 4px 16px ${alpha(NOIR.navyField, 0.25)}`,
            "&:hover": {
              boxShadow: `0 6px 22px ${alpha(NOIR.navyField, 0.45)}`,
            },
          }}
        >
      ...
      {/* Bottom-Left Corner HUD: Services Matrix (Background-less) */}
      <Box
        sx={{
          position: { xs: "relative", md: "absolute" },
          bottom: { md: 64 },
      ...
      >
        <Typography sx={{ fontFamily: MONO, fontSize: "0.65rem", letterSpacing: "0.16em", color: "text.secondary", textTransform: "uppercase" }}>
          01 // SERVICES MATRIX
        </Typography>
        <Stack direction="row" spacing={1.5} useFlexGap flexWrap="wrap" justifyContent="flex-start">
          {CONTENT.services.map((service) => (
            <ServiceChipHover key={service.id} service={service} onOpen={() => handleOpenDrawer(service.id)} />
          ))}
        </Stack>
      </Box>

      {/* Bottom-Right Corner HUD: Interactive Scroll Cue */}
      <Box
        sx={{
          position: { xs: "relative", md: "absolute" },
          bottom: { md: 64 },
...
function ClosingSection() {
  ...
  return (
    <StageSection section={homeSection("closing")}>
      <Reveal>
        <Typography variant="h1" sx={{ color: "primary.main", maxWidth: 1000, mx: "auto", textAlign: "center" }}>
          {CONTENT.closing.statement}
        </Typography>
      </Reveal>
      {/* Farewell: the address lives here, at the very end of the page. */}
      <Reveal delay={0.15}>
        <Stack spacing={1.5} sx={{ mt: { xs: 6, md: 10 }, textAlign: "center", alignItems: "center" }}>
          <Button
            onClick={handleClick}
            ...
            sx={{
              ...
              color: hovered ? "primary.contrastText" : "primary.main",
              borderColor: "primary.main",
              "&:hover": {
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "24px", position: "relative" }}>
